...
const (
	TmuxServerModeDefault   = "default"
	TmuxServerModeDedicated = "dedicated"
)

// Tmux selects the server namespace used by the local interactive carrier.
// The session name remains a fixed Tmux-domain contract.
type Tmux struct {
...
func Default() Config {
	return Config{
		...
		Tmux: Tmux{ServerMode: TmuxServerModeDefault},
	}
}
...
func (config Config) Validate() error {
	...
	if config.Tmux.ServerMode != TmuxServerModeDefault &&
		config.Tmux.ServerMode != TmuxServerModeDedicated {
		return fmt.Errorf(
			"tmux.server_mode must be %q or %q",
			TmuxServerModeDefault, TmuxServerModeDedicated,
		)
	}
