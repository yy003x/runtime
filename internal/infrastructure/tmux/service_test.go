...
func TestSourceTmuxConfigMatchesValidatedFixture(t *testing.T) {
	value, err := os.ReadFile(filepath.Join(reporoot.Root(t), "release", "tmux.conf"))
	if err != nil {
		t.Fatal(err)
	}
	if string(value) != "# Dedicated mode owns this isolated tmux server. User configuration is never loaded.\n"+
		testTmuxConfig {
...
func TestServiceDefaultServerSharesNativeTmuxAndPreservesForeignSession(t *testing.T) {
	if _, err := exec.LookPath("tmux"); err != nil {
		t.Skip("tmux is not installed")
	}
	service, tmuxEnv := newDefaultTestService(t, true)
	runNativeTmux(t, tmuxEnv, "new-session", "-d", "-s", "foreign")
	runNativeTmux(t, tmuxEnv, "new-session", "-d", "-s", "sn-session-other")
	targetPath := buildTestTarget(t, service.home)
	first := startTestTarget(
		t, service, targetPath,
		filepath.Join(service.home, "default-first.json"), "",
	)
	second := startTestTarget(
		t, service, targetPath,
		filepath.Join(service.home, "default-second.json"), "",
	)
	if first.Window.TmuxID == second.Window.TmuxID {
		t.Fatalf("duplicate default-mode tmux_id: %#v %#v", first, second)
	}
	output := runNativeTmux(t, tmuxEnv, "list-sessions", "-F", "#{session_name}")
	for _, expected := range []string{"foreign", "sn-session-other", SessionName} {
		if !strings.Contains(output, expected+"\n") {
			t.Fatalf("native tmux sessions = %q; missing %s", output, expected)
		}
	}
	windows, err := service.List(context.Background())
	if err != nil || len(windows) != 2 {
		t.Fatalf("default-mode windows = %#v, error=%v", windows, err)
	}
	if _, err := service.Stop(context.Background(), first.Window.TmuxID); err != nil {
		t.Fatal(err)
	}
	if output := runNativeTmux(
		t, tmuxEnv, "has-session", "-t", "="+SessionName,
	); output != "" {
		t.Fatalf("unexpected has-session output: %q", output)
	}
	if _, err := service.Stop(context.Background(), second.Window.TmuxID); err != nil {
		t.Fatal(err)
	}
	command := exec.Command("tmux", "-L", "default", "has-session", "-t", "="+SessionName)
	command.Env = tmuxEnv
	if err := command.Run(); err == nil {
		t.Fatal("sn-session remained after its final managed window stopped")
	}
	runNativeTmux(t, tmuxEnv, "has-session", "-t", "=foreign")
	runNativeTmux(t, tmuxEnv, "has-session", "-t", "=sn-session-other")
}

func TestDefaultServerRejectsForeignSnSession(t *testing.T) {
	if _, err := exec.LookPath("tmux"); err != nil {
		t.Skip("tmux is not installed")
	}
	service, tmuxEnv := newDefaultTestService(t, true)
	runNativeTmux(t, tmuxEnv, "new-session", "-d", "-s", SessionName)
	targetPath := buildTestTarget(t, service.home)
	_, err := service.Start(context.Background(), StartRequest{
		Invocation: Invocation{
			ProfileID: "test-cli", Path: targetPath, Argv: []string{targetPath},
			Environment: []string{"ONLY_CONFIGURED=yes"}, CWD: service.home,
			ConfigDigest: digestString("foreign-session-conflict"),
		},
	})
	if err == nil || !strings.Contains(err.Error(), "is not owned by this Runtime home") {
		t.Fatalf("foreign sn-session error = %v", err)
	}
}

func TestServerModeSwitchRequiresQuiescence(t *testing.T) {
	if _, err := exec.LookPath("tmux"); err != nil {
		t.Skip("tmux is not installed")
	}
	home := t.TempDir()
	tmuxTmp, err := os.MkdirTemp("/tmp", "sn-tmux-switch-")
	if err != nil {
		t.Fatal(err)
	}
	...
	dedicatedConfig := testConfig(t, home, true)
	dedicatedConfig.ServerEnv = append(os.Environ(), "TMUX_TMPDIR="+tmuxTmp)
	dedicated, err := NewService(dedicatedConfig)
	if err != nil {
		t.Fatal(err)
	}
	defaultConfig := dedicatedConfig
	defaultConfig.ServerMode = ServerModeDefault
	defaultConfig.ServerEnv = append(os.Environ(), "TMUX_TMPDIR="+tmuxTmp)
	normal, err := NewService(defaultConfig)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
		dedicated.forceKillServer(context.Background())
		normal.forceKillServer(context.Background())
		command := exec.Command("tmux", "-L", "default", "kill-server")
		command.Env = defaultTmuxEnv(tmuxTmp)
		_ = command.Run()
		_ = os.RemoveAll(tmuxTmp)
	})
	targetPath := buildTestTarget(t, home)
	dedicatedWindow := startTestTarget(
		t, dedicated, targetPath, filepath.Join(home, "switch-dedicated.json"), "",
	)
	if _, err := normal.List(context.Background()); err == nil ||
		!strings.Contains(err.Error(), "dedicated Tmux windows still exist") {
		t.Fatalf("default mode accepted live dedicated windows: %v", err)
	}
	if _, err := dedicated.Stop(
		context.Background(), dedicatedWindow.Window.TmuxID,
	); err != nil {
		t.Fatal(err)
	}
	defaultWindow := startTestTarget(
		t, normal, targetPath, filepath.Join(home, "switch-default.json"), "",
	)
	if _, err := dedicated.List(context.Background()); err == nil ||
		!strings.Contains(err.Error(), "default Tmux windows still exist") {
		t.Fatalf("dedicated mode accepted live default windows: %v", err)
	}
	if _, err := normal.Stop(
		context.Background(), defaultWindow.Window.TmuxID,
	); err != nil {
		t.Fatal(err)
	}
}
...
func TestDefaultAttachUsesResolvedNativeSocket(t *testing.T) {
	if _, err := exec.LookPath("tmux"); err != nil {
		t.Skip("tmux is not installed")
	}
	t.Setenv("TERM", "xterm-256color")
	service, _ := newDefaultTestService(t, true)
	targetPath := buildTestTarget(t, service.home)
	started := startTestTarget(
		t, service, targetPath,
		filepath.Join(service.home, "default-attach-target.json"), "",
	)
	master, terminal, err := pty.Open()
	if err != nil {
		t.Fatal(err)
	...
	go func() {
			...
			ctx, started.Window.TmuxID,
			TTYFiles{Stdin: terminal, Stdout: terminal, Stderr: terminal},
		)
	}()
	deadline := time.Now().Add(3 * time.Second)
	for time.Now().Before(deadline) {
		if _, err := master.Write([]byte{0x02, 'd'}); err == nil {
			...
			case attachErr := <-done:
				if attachErr != nil {
					t.Fatalf("default attach: %v", attachErr)
				}
				if _, err := service.Stop(
					context.Background(), started.Window.TmuxID,
				); err != nil {
	...
	t.Fatal("default tmux attach did not detach before timeout")
}
...
func newDefaultTestService(t *testing.T, helper bool) (*Service, []string) {
	t.Helper()
	home := t.TempDir()
	tmuxTmp, err := os.MkdirTemp("/tmp", "sn-tmux-test-")
	if err != nil {
		t.Fatal(err)
	}
	if err := os.Chmod(tmuxTmp, 0o700); err != nil {
	...
	config := testConfig(t, home, helper)
	config.ServerMode = ServerModeDefault
	config.ServerEnv = append(os.Environ(), "TMUX_TMPDIR="+tmuxTmp)
	service, err := NewService(config)
	if err != nil {
		t.Fatal(err)
	}
	tmuxEnv := defaultTmuxEnv(tmuxTmp)
	t.Cleanup(func() {
		service.forceKillServer(context.Background())
		command := exec.Command("tmux", "-L", "default", "kill-server")
		command.Env = tmuxEnv
	...
	return service, tmuxEnv
}

func defaultTmuxEnv(tmuxTmp string) []string {
	result := make([]string, 0, len(os.Environ())+1)
	for _, value := range os.Environ() {
		name, _, _ := strings.Cut(value, "=")
		if name == "TMUX" || name == "TMUX_PANE" || name == "TMUX_TMPDIR" {
			continue
		}
		result = append(result, value)
	}
	return append(result, "TMUX_TMPDIR="+tmuxTmp)
}

func runNativeTmux(t *testing.T, environment []string, args ...string) string {
	t.Helper()
	command := exec.Command("tmux", append([]string{"-L", "default"}, args...)...)
...
func testConfig(t *testing.T, home string, helper bool) Config {
	...
	config := Config{
		Home: home, ServerMode: ServerModeDedicated,
		LockFile:       filepath.Join(home, "state", "tmux.lock"),
