...
func NewService(config Config) (*Service, error) {
	...
	if config.ServerMode != ServerModeDefault &&
		config.ServerMode != ServerModeDedicated {
		return nil, runtimeError(
			contract.ErrorInvalidRequest, contract.PhaseRequest,
			"Tmux server mode must be %q or %q",
			ServerModeDefault, ServerModeDedicated,
		)
	}
...
func (service *Service) Start(
	...
	defer lock.Close()
	if err := service.prepare(ctx, service.dedicated()); err != nil {
		return StartResult{}, err
	}
...
func (service *Service) List(ctx context.Context) ([]Window, error) {
	...
	if err := service.ensureOppositeModeInactive(ctx); err != nil {
		return nil, err
	}
	exists, err := service.managedSessionExists(ctx)
	if err != nil {
...
func (service *Service) send(
	...
	condition := service.registeredIdentityCondition(marker, value)
...
func (service *Service) Stop(
	...
	condition := service.orphanIdentityCondition(marker, value)
	if value.Record != nil {
		condition = service.registeredIdentityCondition(marker, value)
	}
	if last {
		...
		condition = tmuxAnd(
			condition,
			tmuxEquals("#{session_windows}", expectedWindows),
		)
	}
...
func (service *Service) prepare(ctx context.Context, createSocketDir bool) error {
	...
	_, err := service.ownerConfigDigest()
	return err
}
...
func (service *Service) dedicated() bool {
	return service.config.ServerMode == ServerModeDedicated
}

func (service *Service) ownerOptionName() string {
	if service.dedicated() {
		return serverOptionName
	}
	return sessionOptionName
}

func (service *Service) ownerConfigDigest() (string, error) {
	if service.dedicated() {
		return service.readConfigDigest()
	}
	return digestString("tmux.server_mode=default"), nil
}

func (service *Service) clientPrefix() []string {
	if service.dedicated() {
		return []string{"-S", service.config.SocketFile}
	}
	return []string{"-L", "default"}
}

func (service *Service) targetSocket(ctx context.Context) (string, error) {
	if service.dedicated() {
		return service.config.SocketFile, nil
	}
	result, err := service.runTmux(
		ctx, nil, "display-message", "-p", "-t", SessionName,
		"#{socket_path}",
	)
	if err != nil {
		return "", err
	...
	if result.ExitCode != 0 {
		return "", tmuxTransportError(
			contract.ErrorProtocol, "resolve default Tmux socket: %v",
			commandFailure("tmux display-message", result),
		)
	}
	value, err := trimOneLine(result.Stdout)
	if err != nil || !filepath.IsAbs(value) {
		return "", tmuxTransportError(
			contract.ErrorProtocol, "default Tmux socket is invalid",
		)
	}
	return filepath.Clean(value), nil
}

func (service *Service) managedSessionExists(ctx context.Context) (bool, error) {
	if service.dedicated() {
		return service.socketExists()
	}
	result, err := service.runTmux(
		ctx, nil, "has-session", "-t", "="+SessionName,
	)
	if err != nil {
		return false, err
	}
	...
	default:
		return false, tmuxTransportError(
			contract.ErrorProtocol, "inspect default Tmux session: %v",
			commandFailure("tmux has-session", result),
		)
	}
}

func (service *Service) defaultSessionOwner(ctx context.Context) (string, error) {
	result, err := service.runWithPrefix(
		ctx, nil, []string{"-L", "default"},
		"show-options", "-q", "-v", "-t", SessionName,
		sessionOptionName,
	)
	if err != nil {
		return "", err
	...
	if result.ExitCode != 0 {
		return "", tmuxTransportError(
			contract.ErrorProtocol, "inspect default Tmux owner: %v",
			commandFailure("tmux show-options", result),
		)
	}
	return trimOneLine(result.Stdout)
}

func (service *Service) ensureOppositeModeInactive(ctx context.Context) error {
	if !service.dedicated() {
		...
		if exists {
			return tmuxTransportError(
				contract.ErrorConflict,
				"dedicated Tmux windows still exist; stop them before switching tmux.server_mode",
			)
		}
		return nil
	}
	encoded, err := service.defaultSessionOwner(ctx)
	if err != nil || encoded == "" {
		return err
	}
	var marker serverMarker
	if decodeErr := decodeOption(encoded, &marker); decodeErr != nil {
		return nil
	}
	if marker.FullHomeDigest == service.homeDigest {
		return tmuxTransportError(
			contract.ErrorConflict,
			"default Tmux windows still exist; stop them before switching tmux.server_mode",
		)
	}
	return nil
}
...
func (service *Service) runTmux(
	...
	return service.runWithPrefix(ctx, stdin, service.clientPrefix(), args...)
}

func (service *Service) runWithPrefix(
	ctx context.Context,
	stdin io.Reader,
	prefix []string,
	args ...string,
...
func (service *Service) identityCondition(
	ownerEncoded string,
	recordEncoded string,
) string {
	return tmuxAnd(
		tmuxEquals("#{"+service.ownerOptionName()+"}", ownerEncoded),
		tmuxEquals("#{"+windowRecordOption+"}", recordEncoded),
		tmuxEquals("#{"+windowCommitOption+"}", "1"),
	)
}

func (service *Service) registeredIdentityCondition(
	marker serverMarker,
	value liveWindow,
) string {
...
func (service *Service) orphanIdentityCondition(
	marker serverMarker,
	value liveWindow,
) string {
	return strictWindowIdentityCondition(
