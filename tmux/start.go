...
// client command queue that creates the dedicated server and commits its
// marker. This avoids a valid-but-empty sentinel server that has no tmux_id
// and therefore cannot be stopped after a caller crash.
...
func (service *Service) createServerWithTarget(
	...
	configDigest, err := service.readConfigDigest()
	if err != nil {
		return serverMarker{}, "", "", err
	}
	...
	args = append(args, managedTargetCommand(
		name, cwd, helperExecutable, helperArgs,
	)...)
...
func (service *Service) recoverCreatedTarget(
	...
	defer cancel()
	encodedMarker, err := service.showGlobalOption(
		ctx, serverOptionName, true,
	)
